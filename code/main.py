...
def center_window(window, parent_widget=None):
    ...
    # Dialogs are often created with desktop-oriented fixed sizes. Clamp them
    # to the usable display so they remain fully reachable on a MacBook.
    max_width = max(1, avail.width() - 32)
    max_height = max(1, avail.height() - 32)
...
def fit_window_to_screen(window, width_ratio=0.96, height_ratio=0.90):
    """Size a top-level window for the usable area of its current display."""
    app = QApplication.instance()
    screen = None
...
class EMGVisualizer(QMainWindow):
    def __init__(self):
        ...
        # --- WORKSTATION HEADER ---
        # Kept separate from the controls so the app reads as a focused
        # instrument panel rather than a dense collection of buttons.
        header = QFrame()
        header.setObjectName("appHeader")
        header.setStyleSheet(
            "QFrame#appHeader {"
            "background: qlineargradient(x1:0, y1:0, x2:1, y2:0, "
            "stop:0 #16102B, stop:0.42 #112B4A, stop:0.72 #1E1B4B, stop:1 #0B192B);"
            "border: 1px solid #7C3AED; border-radius: 14px; }"
            "QFrame#appHeader QLabel { background: transparent; border: none; }"
        )
        header_layout = QHBoxLayout(header)
        header_layout.setContentsMargins(18, 12, 18, 12)
        header_layout.setSpacing(12)
        wordmark_layout = QVBoxLayout()
        wordmark_layout.setSpacing(1)
        title = QLabel("BIOWAVE")
        title.setStyleSheet("font-size: 25px; font-weight: 800; letter-spacing: 2px; color: #F8FAFC;")
        subtitle = QLabel("REAL-TIME EMG  •  SIGNAL INTELLIGENCE WORKSTATION")
        subtitle.setStyleSheet("font-size: 11px; font-weight: 700; letter-spacing: 1px; color: #67E8F9;")
        wordmark_layout.addWidget(title)
        wordmark_layout.addWidget(subtitle)
        header_layout.addLayout(wordmark_layout)
        header_layout.addStretch(1)
        header_mode = QLabel("●  LIVE WORKSPACE")
        header_mode.setStyleSheet(
            "background-color: #2A1649; color: #E879F9; border: 1px solid #A855F7; "
            "border-radius: 11px; padding: 6px 10px; font-size: 11px; font-weight: 800;"
        )
        header_layout.addWidget(header_mode, 0, Qt.AlignVCenter)
        ...
        # The instrumentation bar contains many precision controls. Keeping
        # it in a horizontal scroller prevents its intrinsic width from
        # forcing the entire window off smaller MacBook displays.
        controls_content = QWidget()
        controls_content.setObjectName("instrumentControls")
        controls_content.setLayout(controls_layout)
        controls_scroll = QScrollArea()
        controls_scroll.setObjectName("instrumentControlsScroll")
        ...
        controls_scroll.setFixedHeight(controls_content.sizeHint().height() + 8)
        self.main_layout.addWidget(controls_scroll)
        ...
        action_content.setObjectName("workflowActions")
        action_content.setLayout(info_actions_layout)
        action_scroll = QScrollArea()
        action_scroll.setObjectName("workflowActionsScroll")
        action_scroll.setWidgetResizable(False)
        ...
        action_scroll.setFixedHeight(action_content.sizeHint().height() + 8)
        self.main_layout.addWidget(action_scroll)
        ...
        # Avoid full Retina viewport repaints when only the incoming traces
        # change. This is especially helpful on high-resolution Mac displays.
        self.plot_widget.setViewportUpdateMode(self.plot_widget.MinimalViewportUpdate)
        ...
        # The plot/status split has a practical minimum width. Scrolling this
        # workspace on compact displays is preferable to letting the native
        # window grow beyond the screen boundary.
        graph_content = QWidget()
        graph_content.setObjectName("graphWorkspace")
        graph_content.setLayout(self.graph_side_layout)
        graph_scroll = QScrollArea()
        graph_scroll.setObjectName("graphWorkspaceScroll")
        graph_scroll.setWidgetResizable(True)
        graph_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        graph_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        graph_scroll.setFrameShape(QFrame.NoFrame)
        graph_scroll.setWidget(graph_content)
        self.main_layout.addWidget(graph_scroll, 1)
    ...
    def showEvent(self, event):
        ...
        if not self._centered_once:
            # On macOS the window is not attached to its final display until
            # after show(). Defer sizing by one event-loop turn so an external
            # or secondary display reports its real available geometry.
            QTimer.singleShot(0, self._fit_and_center_on_current_screen)
            self._centered_once = True

    def _fit_and_center_on_current_screen(self):
        fit_window_to_screen(self, width_ratio=0.96, height_ratio=0.90)
        center_window(self, None)


if __name__ == "__main__":
    if sys.platform == "darwin":
        # Retina icons stay sharp and Qt coalesces bursts of input events,
        # leaving more UI time for live plot updates.
